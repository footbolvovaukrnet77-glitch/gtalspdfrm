...
## Запуск наблюдателя (фиксация сбоев)

Подробно: [docs/ru/WATCHER.md](docs/ru/WATCHER.md). Он читает логи и не трогает
игру. С машины ничего не уходит, пока не указан `--publish`, а тот отказывает на
публичном репозитории, пока не добавлен `--public-ok`.

| Команда | Что делает |
| --- | --- |
| `./tools/run-watcher.sh` | Смотреть за логами и записывать всё, что сломается |
| `./tools/run-watcher.sh --screenshot` | Ещё и снимать экран — игра должна быть в окне или окне без рамки |
| `./tools/run-watcher.sh --rules` | Что считается сбоем и что каждое означает |
| `./tools/run-watcher.sh --publish --public-ok` | Отправлять записи в ветку `diagnostics`, подтвердив, что репозиторий публичный |
| `./tools/run-watcher.sh --help` | Все ключи |

В Windows: `tools\run-watcher.bat`.
