...
| `Jumping` | `TASK_JUMP`, on the transition into the flag | The reason it used to give for not applying — that a jump arrives too late to be worth issuing — does not survive the numbers: the interpolation delay is 120 ms and a jump is close to a second. What was actually wrong was issuing it every frame the flag was set, which restarts the task continuously so it never plays. Swallowed while the ped is being hard-corrected: a jump started on the frame a ped is teleported is a jump from the wrong place |
| `Climbing` | `TASK_CLIMB`, on the transition | Same rule, and it wins over a jump arriving in the same frame, because a ped climbing a wall is also leaving the ground |
...
| `Melee` | `TASK_COMBAT_PED` against the replicated target | The missing half now travels: `CharacterEntity.MeleeTargetId`, written only while the flag is set, validated by the server against its own world, and resolved back to a local ped by the receiving client. Still not issued when the target resolves to nothing — a player too far away to have been built here, or one who has left — which is the case the old entry described and the only one left |
| `InCover` | `TASK_PUT_PED_DIRECTLY_INTO_COVER` at the player's own position, on change | The first half of the old reason is true and the second does not follow: the point does not have to be guessed. A player in cover is standing at their cover, and their position is already on the wire. What remains true is that the receiving client may find no cover there — a prop it does not have, or half a metre of drift onto the wrong side of a wall — and the native then does nothing, which is the old outcome and not a worse one |
| `Parachuting` | `TASK_PARACHUTE` on opening, `CLEAR_PED_TASKS` on closing | Closing is the one transition applied even mid-correction: a ped left under a canopy it no longer has drifts down a street it should be standing in, and a position correction cannot fix that, because the parachute is a task and not a position |
