...
| Server-initiated moves | ✅ authority hold, so a teleport or respawn is not dragged back by in-flight client updates. The wanted level is held the same way: a restored save or an admin's `wanted` command survives the client's next report, and reaches the game |
...
| `dotnet test -c Release` | All 545 tests, with the `.trx` uploaded as an artifact so a failure is readable without re-running anything |
