...
Ожидается: `no broken links, no missing translations, protocol version agrees with
the code`. Проверка падает на относительной ссылке на несуществующий файл, на
`#якоре`, называющем отсутствующий заголовок, на английском документе без русского
двойника или наоборот, а также на версии протокола, напечатанной в документации и не
совпадающей с той, которую на самом деле отправляет `ProtocolConstants`. Нужен только
