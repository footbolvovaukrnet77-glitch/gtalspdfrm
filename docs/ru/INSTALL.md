...
Ожидается: `Passed! - Failed: 0, Passed: 553`.
...
Ожидается: `no broken links, no missing translations, protocol version and test count
agree with the code`. Проверка падает на относительной ссылке на несуществующий файл,
на `#якоре`, называющем отсутствующий заголовок, на английском документе без русского
двойника или наоборот, на версии протокола, напечатанной в документации и не
совпадающей с той, которую на самом деле отправляет `ProtocolConstants`, и на числе
тестов, напечатанном в документации и не совпадающем с тем, сколько их в наборе.
Нужен только Python 3 — устанавливать пакеты не требуется.

Последние две проверки существуют потому, что документация утверждала числа, которые
никто не сверял, и оба уже разъехались: документы протокола описывали версию 5
протокола версии 8. Число в документе, которому не может возразить ни один шаг
сборки, — это число, которое рано или поздно окажется неверным.
