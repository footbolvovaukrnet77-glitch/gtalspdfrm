...
Ожидается: `Passed! - Failed: 0, Passed: 387`.
...
Ожидается: `no broken links, no missing translations`. Проверка падает на
относительной ссылке на несуществующий файл, на `#якоре`, называющем
отсутствующий заголовок, и на английском документе без русского двойника или
наоборот. Нужен только Python 3 — устанавливать пакеты не требуется.
